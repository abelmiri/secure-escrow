...
import {createContext, type Dispatch, type ReactNode, useEffect, useLayoutEffect, useReducer, useRef, useState} from "react"
...
function AuthProvider({children}: { children: ReactNode }) {
    ...
    const [isAuthenticatingCode, setIsAuthenticatingCode] = useState<false | string>(setInitialIsAuthenticating)
    ...
    function setInitialIsAuthenticating() {
        ...
        return !authState.user && code && typeof code === "string" && session_state && iss ? code : false
    }

    useEffect(() => {
        if (isAuthenticatingCode) {
            if (window.opener) {
                window.opener.postMessage({code: isAuthenticatingCode}, window.location.origin)
                window.close()
            }
            else {
                // probably, deep link from TWA got the redirect, or regular comeback from SSO
                getTokenAfterRedirect({code: isAuthenticatingCode, authDispatch}).then(() => setIsAuthenticatingCode(false))
            }
        }
    }, [])

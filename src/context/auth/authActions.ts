...
import type {LoginType, SetUserActionType, UpdateUserType, UserType} from "@/context/auth/AuthType"
...
function updateUser({data, cancelToken, authDispatch}: { data: Partial<UpdateUserType>; cancelToken?: RefObject<AbortController | null>; authDispatch: Dispatch<SetUserActionType> }) {
    return request.patch({url: API_URLS.profile, data, cancelToken}).then((user: UserType) => {
        setUser({data: {user}, authDispatch})
    })
}
...
function _setCookies({res}: { res: LoginType }) {
    ...
    cookieHelper.setItem(COOKIE_VALUES.ACCOUNT.token, `${token_type} ${access_token}`)
    cookieHelper.setItem(COOKIE_VALUES.ACCOUNT.refresh_token, refresh_token)
    cookieHelper.setItem(COOKIE_VALUES.ACCOUNT.token_expires_in, new Date(Date.now() + expires_in * 1000).toString())
}
...
function refreshToken() {
    ...
    return new Promise((resolve, reject) => {
        request
            .post({data, url: API_URLS.getToken, subdomain: "oAuth", dontToast: true})
            .then((res: LoginType) => {
                _setCookies({res})
                resolve(null)
            })
...
function verifyOAUTH({data: {code, code_verifier, redirect_uri}, authDispatch}: { data: { code: string; code_verifier: string; redirect_uri: string }; authDispatch: Dispatch<SetUserActionType> }) {
    const data = new URLSearchParams()
    ...
    data.append("client_id", OAUTH.CLIENT_ID)
    return request.post({url: API_URLS.getToken, subdomain: "oAuth", data}).then(res => {
        _setCookies({res})

        return getProfile({authDispatch}).then(() => {
            resetDataManager.resetData({isAfterLogin: true})
        })
            .catch((err) => {
                Object.values(COOKIE_VALUES.ACCOUNT).forEach(key => cookieHelper.removeItem(key))
                throw err
            })
    })
}
...
function logout() {
    ...
    return request.post({url: API_URLS.logout, subdomain: "oAuth", data, dontToast: true})
}

const authActions = {getProfile, updateUser, setUser, refreshToken, verifyOAUTH, logout}

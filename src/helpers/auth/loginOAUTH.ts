...
export function getTokenAfterRedirect({
  code,
  authDispatch,
}: {
  code: string
  ...
  return new Promise((resolve, reject) => {
    const redirectUrl = sessionStorage.getItem(
      SESSION_STORAGE_VALUES.redirectUrl,
    )
    const codeVerifier = sessionStorage.getItem(
      SESSION_STORAGE_VALUES.codeVerifier,
    )
    ...
    if (redirectUrlVar && codeVerifierVar && window.authDispatch) {
      ...
      authActions
        .verifyOAUTH({
          data: {
            code,
            code_verifier: codeVerifierVar,
            redirect_uri: redirectUrlVar,
          },
...
function loginOAUTH(props?: { redirect?: boolean }) {
  ...
  storeCodeVerifierAndRedirectUrl({
    isLoginByTab: !!isLoginByTab,
    data: { redirectUrl, codeVerifier },
  })
      ...
      const state = btoa(
        encodeURIComponent(
          createQueryString({
            params: {
              ...parseQueryString(),
              isLoginByTab: isLoginByTab ? "true" : null,
            },
          }),
        ),
      )
      const redirectLocation = urlMaker({
        url: API_URLS.login({ codeChallenge, redirectUrl, state }),
        subdomain: "oAuth",
      })
      if (isLoginByTab) {
...
function storeCodeVerifierAndRedirectUrl({
  isLoginByTab,
  data: { redirectUrl, codeVerifier },
}: {
  isLoginByTab: boolean
  data: { redirectUrl: string; codeVerifier: string }
}) {
  if (isLoginByTab) {
